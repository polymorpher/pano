...
export const tokenIdToPosition = (
  tokenId: bigint,
  tickSpacing: TickSpacing,
  poolMapping: Record<string, Address>
): Position => {
...
export const calculateTokenId = ({
  ...
  id |= legs[1] ? calculateLegId(legs[1], tickSpacing) << (64n + 48n) : 0n
  id |= legs[2] ? calculateLegId(legs[2], tickSpacing) << (64n + 48n + 48n) : 0n
  id |= legs[3]
    ? calculateLegId(legs[3], tickSpacing) << (64n + 48n + 48n + 48n)
    : 0n
...
export const AmountSelector = ({
  ...
  const goBackPrompt = allowZero
    ? '(enter x to go back)'
    : '(enter 0 or x to go back)'
  return (
    <Box marginTop={1} flexDirection={'column'}>
      {typeof intro === 'string' ? <Text>{intro}</Text> : intro}
      <Box marginY={1}>
        <Text>
          {prompt} {hideGoBackPrompt ? '' : goBackPrompt}:{' '}
        </Text>
        <TextInput
          focus={!inactive && disabled}
          showCursor
...
export const InProgressSelector = ({
  intro,
  prompt,
  onExit,
  inactive
}: InProgressSelectorProps) => {
  const [textInput, setTextInput] = useState<string>('')
  const { disabled } = useContext(UserInputContext)
  const { addMessage } = useContext(NotificationContext)
  const onSubmit = useCallback(
