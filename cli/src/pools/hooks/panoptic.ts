  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      for (const pair of initPairs) {
        ...
        if (!token0Address) {
          addMessage(`Unknown asset ${token0} on network ${network?.name}`, {
            color: 'red'
          })
          continue
        }
        if (!token1Address) {
          addMessage(`Unknown asset ${token1} on network ${network?.name}`, {
            color: 'red'
          })
          continue
        }
        const uniswapPoolAddress = await uniswapFactory.read.getPool([
          token0Address,
          token1Address,
          fee
        ])
        if (uniswapPoolAddress === zeroAddress) {
          addMessage(
            `Cannot find Uniswap pool address for pair ${pairToStr(token0, token1, fee, network)} `,
            { color: 'red' }
          )
          continue
        }
        const panopticPoolAddress = await panopticFactory.read.getPanopticPool([
          uniswapPoolAddress
        ])
        if (panopticPoolAddress === zeroAddress) {
          addMessage(
            `Panoptic pool is not created for pair ${pairToStr(token0, token1, fee, network)}`,
            { color: 'red' }
          )
          continue
        }
      ...
      addMessage(
        `${validatedPairs.length} pairs validated. Initial pair validation completed.`
      )
    }
    init().catch((ex) => {
...
export const usePoolContractFromPair = (
  pair?: PairedPoolAddresses
): PoolContracts => {
  const { addMessage } = useContext(NotificationContext)
  ...
  useEffect(() => {
    async function init() {
      ...
      const pp = getContract({
        address: pair.panopticPoolAddress,
        abi: PanopticPoolAbi,
        client
      })
      setPanopticPool(pp)
...
export const usePoolStatsByContracts = ({
  panopticPool,
  uniswapPool
}: PoolContracts): PanopticPoolInfo => {
  const { addMessage } = useContext(NotificationContext)
  const { collateral0, collateral1 } = useCollateralAddresses(panopticPool)

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [{ priceTick, recentPriceTicks }, setPriceTickInfo] = useState<{
    priceTick: number
    recentPriceTicks: number[]
  }>(EmptyPriceTickInfo)
  ...
  const recentPrices = recentPriceTicks.map((t) =>
    tickToPrice(t, c1Info.decimals - c0Info.decimals)
  )
  ...
  useEffect(() => {
    async function getStats() {
      ...
      setPriceTickInfo({
        priceTick: medianTick,
        recentPriceTicks: priceArray as number[]
      })
    }
...
export const useAccountPoolFunctions = ({ panopticPool }: PoolContracts) => {
  ...
  const calculateAccumulatedFeesBatch = useCallback(
    async (
      positionIds: bigint[]
    ): Promise<undefined | PremiumValuesWithBalanceAndUtilization> => {
      if (!panopticPool || !wallet.address) {
        return undefined
      }
      const [premium0, premium1, balancesAndUtilizations] =
        await panopticPool.read.calculateAccumulatedFeesBatch([
          wallet.address,
          positionIds
        ])
      const balanceMap: Record<Hex, PositionData> = {}
      for (const [tokenId, balanceWithUtilization] of balancesAndUtilizations) {
        const { balance, utilization0, utilization1 } =
          parseBalanceWithUtilization(balanceWithUtilization)
