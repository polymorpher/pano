...
interface SFPMContextProps {
  simulateBurn: (pool: Address, position: PositionWithData) => Promise<SimulateBurnResult | undefined>
  computeIntrinsicValue: (pool: Address, positions: PositionWithData[]) => Promise<Record<Hex, BigInt01>>
  sfpm: SFPM | undefined
}

export const SFPMContext = createContext<SFPMContextProps>({
  simulateBurn: async (pool: Address, position: PositionWithData): Promise<SimulateBurnResult | undefined> => { return undefined },
  computeIntrinsicValue: async (pool: Address, positions: PositionWithData[]): Promise<Record<Hex, BigInt01>> => { return {} },
  sfpm: undefined
})
...
export const SFPMProvider = ({ children }: PropsWithChildren) => {
  ...
  return <SFPMContext.Provider value={ { sfpm, simulateBurn, computeIntrinsicValue }}>
    {children}
