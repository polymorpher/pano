...
import {
  archivePoolingBlockRangeSize,
  archivePoolingInterval
} from '../config.js'
import { type Address, parseAbiItem } from 'viem'
import { extractLR64 } from '../common.js'
import { ScalingFactor } from '../util.js'

const OptionMinted = parseAbiItem(
  'event OptionMinted(address indexed recipient, uint128 positionSize, uint256 indexed tokenId, int24 tickAtMint, uint128 poolUtilizations)'
)
...
export const useScanPositions = (options?: UseScanPositionsOptions) => {
  ...
  const scan = useCallback(
    async (
      ...
      const firstBlock =
        duration < 0
          ? finalBlock + BigInt(duration)
          : finalBlock -
            BigInt(
              blockTime && blockTime > 0
                ? Math.ceil(duration / blockTime)
                : duration
            )
      const poolingInterval = options?.poolingInterval ?? archivePoolingInterval
      const blockRangeSize =
        options?.blockRangeSize ?? archivePoolingBlockRangeSize
      if (
        !(await onChunkScanned({
          entries: [],
          fromBlock: 0n,
          toBlock: 0n,
          firstBlock,
          finalBlock,
          numBlocksScanned: 0n,
          totalBlocks: finalBlock - firstBlock
        }))
      ) {
        return
      }
      for (let b = firstBlock; b < finalBlock; b += blockRangeSize) {
        const toBlock =
          finalBlock < b + blockRangeSize ? finalBlock : b + blockRangeSize
        const fromBlock = b
        try {
          ...
          const timestampsEntries: Array<[string, bigint]> = await Promise.all(
            logs.map(async (log) => {
              const b = await archiveClient.getBlock({
          ...
          const shouldContinue = await onChunkScanned({
            entries,
            firstBlock,
            finalBlock,
            fromBlock,
            toBlock,
            totalBlocks: finalBlock - firstBlock,
            numBlocksScanned: toBlock - firstBlock
          })
          if (!shouldContinue) {
            return
          }
